...
<script>
import {getTeacherMineInfo,changeTeacherMinePass} from "@/api";
...
export default {
  ...
  mounted(){
    getTeacherMineInfo({userId:global.UserId}).then(({data})=>{
  ...
  methods:{
    ...
    editPasswordSubmit(){
      ...
      if(this.editForm.oldPassIn !== this.editForm.oldPass) {
        ...
        console.log('---------');
        changeTeacherMinePass({newPassword:this.editForm.newPassword,userId:this.editForm.userId}).then(({data})=>{
          if(data.code === 0){
            this.$message({
              message: '密码修改成功',
              type: 'success'
